...
file_request(Request, Module, Handle) ->
    ...
    case (catch apply(Module, Command, Args ++ [Handle])) of
        {'EXIT', {undef, _}} ->
            case file_request(Request, virtualfs_iolist_io_server, Module:io_list(Handle)) of
...
io_request(Request, Module, Handle) ->
    ...
    case (catch apply(Module, Command, Args ++ [Handle])) of
        {'EXIT', {undef, _}} ->
            case io_request(Request, virtualfs_iolist_io_server, Module:io_list(Handle)) of
                {Reply, Result, _Handle1} ->
                    {Reply, Result, Handle};
                {stop, Reason, Result, _Handle1} ->
                    {stop, Reason, Result, Handle}
